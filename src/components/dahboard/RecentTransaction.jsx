    ...
    <div className="w-full lg:w-[40%] ">
      <div
        className={`${darkMode ? "bg-gray-800" : "bg-white"
          } w-full h-auto p-5 rounded-xl shadow overflow-hidden transition-colors duration-200`}
      >
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-1 text-sm font-medium ${darkMode
            ...
            {showFilterDropdown && (
              <div
                className={`absolute right-0 mt-1 w-40 ${darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-100"
                  } shadow-lg rounded-lg py-1 z-10 border`}
              >
                <button
        ...
        <div
          className={`${showAllTransactions
            ? "custom-scrollbar overflow-y-auto h-[calc(100%-80px)]"
            : "overflow-hidden"
            }`}
        >
          {filteredTransactions.length > 0 ? (
            displayedTransactions.map((item) => (
              <div
                key={item.id}
                className={`border-b ${darkMode ? "border-gray-700" : "border-gray-100"} py-3`}
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div
                      className={`px-1.5 py-1 rounded-full ${darkMode ? "bg-gray-700" : "bg-gray-100"
                        }`}
                    >
                  ...
                  <div
                    className={`flex justify-center items-center gap-2 text-xs font-semibold p-2 rounded-lg ${item.type === "income"
                      ? darkMode
