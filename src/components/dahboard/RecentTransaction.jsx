  ...
  return (
    <div className="w-full h-full">
      ...
      >
        <div className="flex justify-between items-center mb-2 md:mb-3 sticky top-0 z-20">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-2 cursor-pointer text-xs md:text-sm font-medium ${darkMode
        ...
        <div
          className={`pr-2 md:pr-3 mt-2 ${showAllTransactions ? "custom-scrollbar overflow-y-scroll" : "overflow-y-hidden"} 
        ...
        >
          {isLoading ? (
            ...
            (showAllTransactions ? filteredTransactions : displayedTransactions).map((item) => (
              <div
                key={item.id}
                className={`border-b ${darkMode ? "border-gray-700 hover:bg-gray-900" : "border-gray-100 hover:bg-gray-200"} py-2 md:py-3 
                  flex justify-between items-center px-1 md:px-2
                 }`}
              >
                <div className="flex items-center gap-2 md:gap-3">
                  <div className={`p-1 md:px-1.5 md:py-1 rounded-full ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}>
                ...
                <div
                  className={`flex justify-center items-center gap-1 md:gap-2 text-xs font-semibold p-1.5 md:p-2 rounded-lg 
                    ${item.type === "income" ? (darkMode ? "text-green-300 bg-green-900" : "text-green-600 bg-green-100")
                ...
                >
                  ...
                  {item.type === "income" ? <FiTrendingUp size={14} className="md:text-base" /> : <FiTrendingDown size={14} className="md:text-base" />}
                </div>
              </div>
            ))
          ) : (
            <div className={`text-center py-8 ${darkMode ? "text-gray-400" : "text-gray-500"} text-xs md:text-sm`}>
              {transactionFilter === "all" ? "Belum ada transaksi" : transactionFilter === "income" ? "Belum ada pemasukan" : "Belum ada pengeluaran"}
            </div>
          )}
        </div>
        ...
        {filteredTransactions.length > maxVisibleTransactions && (
          <button
            ...
            className={`mt-2 md:mt-3 ${darkMode ? "text-purple-400 hover:text-white border-purple-600 hover:bg-purple-700"
